...
OPENSLIDE_PROPS_FIXTURES.forEach(fixture => {
  describe(`fixture [${fixture.source}]`, () => {
    let inputStream = Fs.createReadStream(
      Path.resolve(__dirname, fixture.source)
    );
    let expectedJson = JSON.parse(
      Fs.readFileSync(Path.resolve(__dirname, fixture.expected), 'utf8')
    );

    it('parses correctly', done => {
      prop2json(inputStream, {}, (err, props) => {
        expect(err).to.be.null();
        expect(props).to.equal(expectedJson);
        done();
      });
    });
  });
